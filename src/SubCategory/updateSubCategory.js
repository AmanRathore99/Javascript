...
const UpdateSubCategoryComponent = (props) =>{
...
useEffect(()=>{
    ...
    axios.get(`http://localhost:8081/api/subcategories/${id}`).then((response)=>{
...
const save = () =>{
    ...
    console.log("in save");
    let id=props.match.params.id;
    axios.put(`http://localhost:8081/api/subcategories/${id}`,subCategory).then((response)=>{
        console.log(response.data.rows);
