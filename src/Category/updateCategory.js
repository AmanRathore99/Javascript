...
const UpdateCategoryComponent = (props) =>{
...
useEffect(()=>{
    ...
    axios.get(`http://localhost:8081/api/category/${id}`).then((response)=>{
...
const save = () =>{
    ...
    let id = props.match.params.id;
    console.log("in save");
    axios.put(`http://localhost:8081/api/category/${id}`,Category).then((response)=>{
        console.log(response.data.rows);
